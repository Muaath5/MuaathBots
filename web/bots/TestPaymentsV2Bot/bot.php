...
# Functions
function GetInvoiceByPayload(string $payload, $settings, bool $returnIndexInArray = false)
{
...
$contact_the_dev = json_encode($contact_the_dev_phpjson);

# Update handlers

function MessageHandler($message, bool $is_channel_post = false)
{
    global $settings;
    ...
    {    
        if ($message->text[0] === '/')
        {
            CommandsHandler($message, $settings);
        }
    }
    else if (property_exists($message, 'successful_payment'))
    {
        $resultInvoice = GetInvoiceByPayload($message->successful_payment->invoice_payload, $settings, true);
        SendMessage(LogsChatID, "JSON for resultInvoice is: <code>" . json_encode($resultInvoice) . '</code>');
        ...
        {
            SendMessage(LogsChatID, "Error, {$message->successful_payment->invoice_payload} was sold out over the limit", 0, $contact_the_dev);
        }
...
Total amount: {$floatTotalAmount} {$message->successful_payment->currency}.";
        ...
        SendMessage(LogsChatID, $info);
            
        SendMessage($senderChat->id, $settings->successful_payment_message, $message->message_id);
    }
}
...
function CommandsHandler($message, $settings)
{
    ...
    {      
        SendMessage($message->chat->id, $settings->start_message, $message->message_id);
    }
    else if ($message->text === '/project' || $message->text === '/start project')
    {
        SendMessage($message->chat->id, $settings->project_message, $message->message_id);
    }
    else if ($message->text === '/help' || $message->text === '/start help')
    {
        SendMessage($message->chat->id, $settings->help_message, $message->message_id);
    }
    else if ($message->text === '/inline' || $message->text === '/start inline')
    {
        ...
        SendMessage($message->chat->id, $settings->inline_message, $message->message_id, json_encode($inlineQueryKeyboard));
    }
    ...
    {   
        ...
        SendMessage($senderChat->id, $settings->warning_message);
        
        $mainInvoice = SendInvoice($senderChat->id, $settings->invoices[0]->title, $settings->invoices[0]->description, $settings->invoices[0]->payload,
...
Which command was used: {$message->text}.";
        ...
        $newInvoiceLogMsg = SendMessage(LogsChatID, $newInvoiceRequestText);
...
Error description: {$mainInvoice->description}.";
            # Log error
            SendMessage(LogsChatID, $errorStr, $newInvoiceLogMsg->message_id, $contact_the_dev);
        }
    }
    else
    {
        for ($i = 0; $i < count($settings->invoices); $i++)
        {
            if ($message->text === "/start {$settings->invoices[$i]->start_param}")
            {
                ...
                $inlineInvoice = SendInvoice($message->chat->id, $settings->invoices[$i]->title, $settings->invoices[$i]->description, $settings->invoices[$i]->payload,
...
Which command was used: {$message->text}.";
                ...
                $inlineInvoiceLogMsg = SendMessage(LogsChatID, $newInvoiceRequestText);
...
Retry after: {$inlineInvoice->parameters->retry_after}.";
                    # Log error
                    SendMessage(LogsChatID, $errorStr, $inlineInvoiceLogMsg->message_id, $contact_the_dev);
                }
                break;
            }
        }
...
function ChannelPostHandler($channel_post)
{
    MessageHandler($channel_post, true);
}

function InlineQueryHandler($inline_query)
{
    global $contact_the_dev;
    global $settings;
    ...
    $answerInlineSuccess = AnswerInlineQuery($inline_query->id, $results, 300, false, '', 'Main invoice', 'invoice');
    ...
    if ($answerInlineSuccess->ok === false)
    {
        ...
        # Logging error
        SendMessage(LogsChatID, "<b>New inline invoice request:</b>
<b>User:</b>
...
function ShippingQueryHandler($shipping_query)
{  
    global $settings;
    global $contact_the_dev;

    $currentInvoice = GetInvoiceByPayload($shipping_query->invoice_payload, $settings);
    ...
    if (count($currentInvoice->supported_countries) === 0 || $countryResult !== false)
    {
        ...
        {
            $shipping = AnswerShippingQuery($shipping_query->id, true, json_encode($currentInvoice->shipping_options));
            if ($shipping === false || $shipping->ok === false)
            {
                # Logging
                SendMessage(LogsChatID, str_replace('{json-error}', $settings->answer_shipping_query_failed_log, json_encode($shipping)), 0, $contact_the_dev);
            }
        }
        else
        {
            ...
            AnswerShippingQuery($shipping_query->id, false, '', $errorMsg);
        }
    }
...
function PreCheckoutQueryHandler($pre_checkout_query)
{
    global $settings;
    $currentInvoice = GetInvoiceByPayload($pre_checkout_query->invoice_payload, $settings);
    
    # Check the limit of the product
    if ($currentInvoice->limit === 0)
    {
        AnswerPreCheckoutQuery($pre_checkout_query->id, false, $settings->error_product_sold_out);
    }
    else
    {
...
function MyChatMemberHandler($my_chat_member)
{
    ...
    if ($my_chat_member->new_chat_member === 'member')
    {
        ...
        {
            SendMessage(LogsChatID, "{$my_chat_member->from->first_name} [{$my_chat_member->from->username}, <code>{$my_chat_member->from->id}</code>] Started conversion with the bot.");
        }
        else
        {
            SendMessage(LogsChatID, "{$my_chat_member->from->first_name} [{$my_chat_member->from->username}, <code>{$my_chat_member->from->id}</code>] Added the bot to chat:
{$my_chat_member->chat->title} [{$my_chat_member->chat->username}, <code>{$my_chat_member->chat->id}</code>]");
    ...
    {
        SendMessage(LogsChatID, "{$my_chat_member->from->first_name} [{$my_chat_member->from->username}, <code>{$my_chat_member->from->id}</code>] Kicked the bot from chat:
{$my_chat_member->chat->title} [{$my_chat_member->chat->username}, <code>{$my_chat_member->chat->id}</code>]");
    }
}
...
function CallbackQueryHandler($callback_query)
{
    ...
    if (StartsWith($callback_query->data, 'Invoice_'))
    {
        $providerData =
        [
        ...
        $requestedInvoice = GetInvoiceByPayload(substr($callback_query->data, 9, strlen($callback_query->data)), $settings);

        $photoHeight = $requestedInvoice->photo_height;
        $photoWidth = $requestedInvoice->photo_width;

        SendInvoice($callback_query->message->chat->id, $requestedInvoice->title, $requestedInvoice->description, $requestedInvoice->payload,
        ...
        AnswerCallbackQuery($callback_query->id, $settings->succesed);
    }
    else
    {
        AnswerCallbackQuery($callback_query->id, $settings->error_unknown_callback_query);
    }
}
    
    

function StartsWith(string $text, string $start)
{
...
function DeleteMessageIfChannel($message)
{
